...
def calculate_irpf(salary, currency):
    # Simulate a calculation based on the inputs
    return f"Net Salary for number: {salary} in currency: {currency}"

if __name__ == "__main__":
    if len(sys.argv) != 3:
        print("Usage: calculate_irpf.py <number> <currency>")
        sys.exit(1)
    
    salary = sys.argv[1]
    currency = sys.argv[2]

    result = calculate_irpf(salary, currency)
    print(result)
